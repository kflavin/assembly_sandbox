...
record_read_loop:
 ...
 pushq $record_buffer
 call read_record
 addq $16, %rsp         # remove the two parameters from the stack

 # Returns the number of bytes read.  If it isn't what we requested, then quit.
 cmpq $RECORD_SIZE, %rax
 jne finished_reading

 # Otherwise print out the first name (but get its size first)
